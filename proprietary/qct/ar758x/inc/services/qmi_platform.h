...
do \
{ \
  pthread_condattr_init(&(signal_ptr)->cond_attr); \
  rc = pthread_condattr_setclock(&(signal_ptr)->cond_attr, CLOCK_MONOTONIC); \
  if(!rc) \
  { \
    rc = pthread_cond_init(&(signal_ptr)->cond_var, &(signal_ptr)->cond_attr); \
    if(!rc) \
    { \
...
#define QMI_PLATFORM_DESTROY_SIGNAL_DATA(signal_ptr) \
  do \
  { \
    pthread_condattr_destroy(&(signal_ptr)->cond_attr); \
