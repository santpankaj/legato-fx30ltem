...
#include "le_kernelModule_server.h"
...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_kernelModule_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_kernelModule_ServerHandlers", 3);
...
static void Handle_le_kernelModule_Load
(
...
static void Handle_le_kernelModule_Unload
(
    le_msg_MessageRef_t _msgRef
...
)
{
    ...
    // Call the function
    le_result_t _result;
    _result  = le_kernelModule_Unload ( 
        moduleName );
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_kernelModule_Load : Handle_le_kernelModule_Load(msgRef); break;
        case _MSGID_le_kernelModule_Unload : Handle_le_kernelModule_Unload(msgRef); break;
