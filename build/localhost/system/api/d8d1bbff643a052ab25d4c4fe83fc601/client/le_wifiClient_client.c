...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("le_wifiClient_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("le_wifiClient_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("le_wifiClient_ClientHandlers", 5);
}
...
static void _Handle_le_wifiClient_AddNewEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    le_wifiClient_NewEventHandlerFunc_t _handlerRef_le_wifiClient_AddNewEventHandler = (le_wifiClient_NewEventHandlerFunc_t)_clientDataPtr->handlerPtr;
    ...
    if ( _handlerRef_le_wifiClient_AddNewEventHandler != NULL )
    {
        _handlerRef_le_wifiClient_AddNewEventHandler(event, contextPtr );
    }
    else
    {
...
)
{
    ...
    _UNLOCK
    handlerRef = (le_wifiClient_NewEventHandlerRef_t)clientDataPtr->handlerRef;
...
)
{
    ...
    // Unpack the result first
    if (!le_pack_UnpackInt16( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    if (bssid)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (bssidSize-1) ));
    }
...
)
{
    ...
    if (ssidPtr)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (*ssidSizePtr) ));
    }
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_wifiClient_AddNewEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_le_wifiClient_AddNewEventHandler, msgRef, clientDataPtr);
            break;
