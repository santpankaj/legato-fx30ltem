...
#include "le_kernelModule_messages.h"
#include "le_kernelModule_interface.h"
...
typedef struct
{
    ...
    le_kernelModule_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "le_kernelModule_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("le_kernelModule_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("le_kernelModule_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("le_kernelModule_ClientHandlers", 5);
}
...
)
{
    ...
    LE_FATAL("Component for le_kernelModule disconnected\n");
}
...
)
{
    ...
    if ( strnlen(moduleName, 60) > 60 )
    {
        LE_FATAL("strnlen(moduleName, 60) > 60");
    }
    ...
    _msgPtr->id = _MSGID_le_kernelModule_Load;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_kernelModule_Unload;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
