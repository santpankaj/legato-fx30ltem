...
)
{
    ...
    _UNLOCK
    handlerRef = (le_mdc_SessionStateHandlerRef_t)clientDataPtr->handlerRef;
...
)
{
    ...
    le_ref_DeleteRef(_HandlerRefMap, handlerRef);
    _UNLOCK
    handlerRef = (le_mdc_MtPdpSessionStateHandlerRef_t)clientDataPtr->handlerRef;
    le_mem_Release(clientDataPtr);
    LE_ASSERT(le_pack_PackReference( &_msgBufPtr, &_msgBufSize,
                                     handlerRef ));
...
)
{
    ...
    _requiredOutputs |= ((!!(connectionStatePtr)) << 0);
...
)
{
    ...
    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(interfaceName)) << 0);
    LE_ASSERT(le_pack_PackUint32(&_msgBufPtr, &_msgBufSize, _requiredOutputs));
    ...
    if (interfaceName)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (interfaceNameSize-1) ));
    }
...
)
{
    ...
    if (ipAddr)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (ipAddrSize-1) ));
    }
...
)
{
    ...
    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(gatewayAddr)) << 0);
    LE_ASSERT(le_pack_PackUint32(&_msgBufPtr, &_msgBufSize, _requiredOutputs));
    ...
    if (gatewayAddr)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (gatewayAddrSize-1) ));
    }
...
)
{
    ...
    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(dns1AddrStr)) << 0);
    _requiredOutputs |= ((!!(dns2AddrStr)) << 1);
    LE_ASSERT(le_pack_PackUint32(&_msgBufPtr, &_msgBufSize, _requiredOutputs));
    ...
    if (dns1AddrStr)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (dns1AddrStrSize-1) ));
    }
    if (dns2AddrStr)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (dns2AddrStrSize-1) ));
    }
...
)
{
    ...
    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(downlinkDataBearerTechPtrPtr)) << 0);
    _requiredOutputs |= ((!!(uplinkDataBearerTechPtrPtr)) << 1);
    LE_ASSERT(le_pack_PackUint32(&_msgBufPtr, &_msgBufSize, _requiredOutputs));
...
)
{
    ...
    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(rxBytesPtr)) << 0);
    _requiredOutputs |= ((!!(txBytesPtr)) << 1);
    LE_ASSERT(le_pack_PackUint32(&_msgBufPtr, &_msgBufSize, _requiredOutputs));
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_mdc_StopBytesCounter;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_mdc_StartBytesCounter;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(apnStr, 100) > 100 )
    {
        LE_FATAL("strnlen(apnStr, 100) > 100");
    }
...
)
{
    ...
    if (apnStr)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (apnStrSize-1) ));
    }
...
)
{
    ...
    if ( strnlen(password, 100) > 100 )
    {
        LE_FATAL("strnlen(password, 100) > 100");
    }
...
)
{
    ...
    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(typePtr)) << 0);
    _requiredOutputs |= ((!!(userName)) << 1);
    _requiredOutputs |= ((!!(password)) << 2);
    LE_ASSERT(le_pack_PackUint32(&_msgBufPtr, &_msgBufSize, _requiredOutputs));
    ...
    if (password)
    {
        LE_ASSERT(le_pack_PackSize( &_msgBufPtr, &_msgBufSize, (passwordSize-1) ));
    }
...
)
{
    ...
    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(profileRefPtr)) << 0);
    LE_ASSERT(le_pack_PackUint32(&_msgBufPtr, &_msgBufSize, _requiredOutputs));
...
)
{
    ...
    // Pack a list of outputs requested by the client.
    uint32_t _requiredOutputs = 0;
    _requiredOutputs |= ((!!(failureTypePtr)) << 0);
    _requiredOutputs |= ((!!(failureCodePtr)) << 1);
    LE_ASSERT(le_pack_PackUint32(&_msgBufPtr, &_msgBufSize, _requiredOutputs));
...
)
{
    ...
    // Range check values, if appropriate
    if ( strnlen(interfaceName, 20) > 20 )
    {
        LE_FATAL("strnlen(interfaceName, 20) > 20");
    }
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_le_mdc_MapProfileOnNetworkInterface;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
