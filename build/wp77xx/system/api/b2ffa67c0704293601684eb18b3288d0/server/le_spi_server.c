...
)
{
    ...
    _ServerDataPool = le_mem_CreatePool("le_spi_ServerData", sizeof(_ServerData_t));

    // Create safe reference map for handler references.
    // The size of the map should be based on the number of handlers defined for the server.
    // Don't expect that to be more than 2-3, so use 3 as a reasonable guess.
    _HandlerRefMap = le_ref_CreateMap("le_spi_ServerHandlers", 3);
...
)
{
    ...
    le_spi_DeviceHandleRef_t handleBuffer;
...
)
{
    ...
    {
        LE_DEBUG("Adjusting readDataSize from %zu to 1024", readDataSize);
        readDataSize = 1024;
    }

    // Define storage for output parameters
    uint8_t readDataBuffer[1024];
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_le_spi_Open : Handle_le_spi_Open(msgRef); break;
        case _MSGID_le_spi_Close : Handle_le_spi_Close(msgRef); break;
        case _MSGID_le_spi_Configure : Handle_le_spi_Configure(msgRef); break;
        case _MSGID_le_spi_WriteReadHD : Handle_le_spi_WriteReadHD(msgRef); break;
        case _MSGID_le_spi_WriteHD : Handle_le_spi_WriteHD(msgRef); break;
        case _MSGID_le_spi_ReadHD : Handle_le_spi_ReadHD(msgRef); break;
        case _MSGID_le_spi_WriteReadFD : Handle_le_spi_WriteReadFD(msgRef); break;
